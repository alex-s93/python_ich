# 1. Напишите функцию, которая принимает список кортежей от пользователя,
# где каждый кортеж содержит информацию о студенте (имя, возраст, средний балл).
# Программа должна вывести на экран имена студентов, чей средний балл выше заданного значения.
# Используйте методы кортежей и циклы для обработки данных.Выведите итоговый список на экран с помощью команды print.
# Пример списка кортежей:
#
# [("Alice", 20, 90), ("Bob", 19, 80), ("Charlie", 21, 95), ("David", 18, 85)]
#
# Пример вывода:
#
# Введите пороговое значение среднего балла: 90
#
# Студенты с средним баллом выше 90 : ['Charlie']

students_list = []

while True:
    string = input("Enter student's name, age and average score separated by comma. " +
                   "Enter 'Stop' if you don't have more information about students: ")
    if string.lower() == 'stop':
        break

    name = string.split(',')[0]
    age = int(string.split(',')[1])
    score = float(string.split(',')[2])
    students_list.append((name, age, score))

threshold = float(input("Enter a threshold for the average score: "))

students_names = []

for student in students_list:
    if student[2] > threshold:
        students_names.append(student[0])

print(f"Student(s) with average score more than threshold {threshold}: {students_names}")

